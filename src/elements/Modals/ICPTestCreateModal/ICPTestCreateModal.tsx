...
import { ImportTestResultsCsv } from "./Parts/ImportTestResultsCsv";

const SCREEN_WIDTH = getAppDimensions().width;

export const ICPTestCreateModal = ({ data, quickMenu, ro }) => {
  ...
  const hasRoTank = tanks?.some((tank) => tank?.type === "rodi_reservoir");

  let allTanks = ro
    ? tanks?.filter((tank) => tank?.type === "rodi_reservoir")
    : tanks?.filter(
        (tank) =>
          !["rodi_reservoir", "saltwater_reservoir"].includes(tank?.type)
      );

  const tank = ro
    ? allTanks?.find((t) => t.id === tankId) ||
      allTanks?.find((t) => t.type === "rodi_reservoir")
    : allTanks?.find((t) => t.id === tankId) ||
  ...
  const elements = useMemo(() => {
    return Object.entries(tank?.testingConfig || {})
      .filter(([_, config]) => config?.icp === true)
      .map(([key, config]) => {
        ...
        return {
          ...
          isRoElement: elementData?.isRoElement,
          readValue: ro
            ? elementData?.isRoElement
              ? 0
              : undefined
            : undefined,
        };
      ...
      .sort((a, b) => {
        if (!a?.label) console.log("Missing label for element:", a);
