...
export const TankReptilesScreen: React.FC = () => {
  ...
  const liveStock = data?.reptile?.data || [];

  const pagination = data?.reptile?.pagination || {};

  const handleLoadMore = async () => {
    const page = (pagination?.current_page || 1) + 1;

    if (pagination?.current_page < pagination?.last_page) {
      await getLiveStock({ page, tankId: data?.id, type: "reptile" });
    }
  };

  const handleInitialLoad = async () => {
    setLoading(true);
    await getLiveStock({ page: 1, tankId: data?.id, type: "reptile" });
    setLoading(false);
  };
  ...
  const handleRemoveFromTank = async (id: string, uuid: string) => {
    ...
    dispatch(
      removeTankLiveStockById({ uuid, tankId: data?.id, type: "reptile" })
    );
  ...
  const handleNavigateBrowse = () =>
    navigate(`${LIVESTOCK_PATH}?filter=is_reptile:true`);

  const renderHeader = () => (
    <>
      <UserPostCardScreenHeader title="Reptiles" icon="reefDocsTurtle" />
      ...
      {liveStock?.length === 0 && !loading && (
        <NoDataFallbackCard
          ...
          icon="reefDocsTurtle"
          description="You haven't added any reptiles yet. Browse Reptiles and add some to your tank!"
          buttonTitle="Browse Reptiles"
          centered={true}
